        ...
        grp_df = df.groupby(['installation_id', 'game_session', 'world']).agg(
            {
                'timestamp': ['max'],
                'level': {
                    'mean': lambda x: None if pd.Series(x).isnull().all() else x.mean(),
                    'std': lambda x: None if pd.Series(x).isnull().all() else x.std(),
                    'max': lambda x: None if pd.Series(x).isnull().all() else x.max(),
                    'min': lambda x: None if pd.Series(x).isnull().all() else x.min(),
                },
                'round': {
                    'mean': lambda x: None if pd.Series(x).isnull().all() else x.mean(),
                    'std': lambda x: None if pd.Series(x).isnull().all() else x.std(),
